...
class ReleaseConfigurationTests(unittest.TestCase):
    ...
    def test_release_uses_a_direct_trusted_pypi_publisher(self):
        ...
        self.assertIn("needs: [build, preflight-pypi]", workflow)
        self.assertEqual(workflow.count("ENABLED: ${{ vars.PYPI_RELEASE_ENABLED }}"), 1)
        ...
        self.assertGreaterEqual(workflow.count("Verify uploaded artifact hashes"), 1)
        self.assertGreaterEqual(workflow.count("tools/verify_index_artifacts.py"), 1)
        ...
        self.assertGreaterEqual(workflow.count("packages-dir: dist/"), 1)
        ...
        self.assertNotIn("TestPyPI", workflow)
        self.assertNotIn("promote-pypi", workflow)
        self.assertNotIn("promote_run_id", workflow)
        self.assertNotIn("promoted-artifact", workflow)
        draft = workflow.index('gh release create "$tag"')
        upload = workflow.index('gh release upload "$tag"')
        download = workflow.index('gh release download "$tag"')
        ...
        self.assertIn("--draft", workflow[draft:upload])
        self.assertIn("--clobber", workflow[upload:download])
        self.assertIn("sha256sum --check SHA256SUMS", workflow)
        self.assertIn("actual != expected", workflow)
        self.assertIn("is already published; refusing to replace it", workflow)
    ...
    def test_release_docs_use_a_draft_first_immutable_github_release(self):
        ...
        self.assertIn("only then publishes the draft", guide)
        self.assertIn("Re-run failed jobs", guide)
        self.assertIn("automatic digest comparison", guide)
        self.assertIn("verifies the recorded distribution hashes", guide)
